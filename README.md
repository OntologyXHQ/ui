...
The canonical plan is `docs/roadmap/UI_TASK_LIST.md`. `UIR00` reset completion truth and quality gates; `UIR01` now provides production-Studio browser acceptance on an installed Chrome/Chromium before any public visual export can be promoted from `candidate` to `accepted`. `UIR02` Foundations reacceptance is now closed: semantic tokens, environment semantics and `UiRoot` nesting/SSR/realm ownership are certified, making `UiRoot` the first post-reset `accepted` export. `UIR03` is now closed: all nine Layout primitives are reaccepted with finite logical APIs, dedicated Studio examples, behavior ownership and component-owned G6 evidence. UIR03-B added finite Grid tracks, semantic Container widths, logical Inset/SafeArea ownership and a strictly decorative one-axis Spacer. `UIR04` is next for typography, Icon, Surface and Divider. Native Wayland/compositor/IME implementations stay in OXS or another host consumer.
