...
export const uiDocs = defineUiDocsGroup([
  {
    ...
    status: 'candidate', accessibility: 'Static rows preserve list semantics; actionable rows use a native button with optional current-state exposure.',
  ...
  },
  {
    ...
    status: 'candidate', accessibility: 'Exposes list/listitem semantics through ListItem children.', rtl: 'Collection flow inherits logical direction.', touch: 'Interactive children own their target policy.', responsive: 'Fills available inline size without viewport assumptions.',
  },
  {
    ...
    status: 'candidate', accessibility: 'Section heading is programmatically associated with its content.', rtl: 'Header copy and trailing utility use logical order.', touch: 'No private interaction ownership.', responsive: 'Header regions wrap inside narrow containers.',
  },
  {
    ...
    status: 'candidate', accessibility: 'Uses the semantic Divider primitive.', rtl: 'Content inset is logical, not left/right specific.', touch: 'Non-interactive.', responsive: 'Tracks collection width.',
  },
  {
  ...
  },
  {
    ...
    status: 'candidate', accessibility: 'Uses tabpanel semantics, aria-labelledby and hidden state while preserving optional mounted content.', rtl: 'Content direction inherits from the surrounding UiRoot.', touch: 'Non-interactive container; descendants own interaction.', responsive: 'Fills available container space without viewport assumptions.',
  },
  {
    ...
    status: 'candidate', accessibility: 'Uses nav landmarks and aria-current for the active destination.', rtl: 'Ordering and icon/copy placement inherit logical direction.', touch: 'Destination controls use Button interaction ownership.', responsive: 'Auto mode changes bar → rail → drawer from container width, not device identity.',
    playground: {
  ...
  },
  {
    ...
    status: 'candidate', accessibility: 'Associates the header with its title and preserves child control semantics.', rtl: 'Leading/title/actions use logical regions.', touch: 'Interactive children preserve shared target policy.', responsive: 'Actions remain bounded while copy can truncate/wrap.',
  },
  {
    ...
    status: 'candidate', accessibility: 'Plain text remains readable by assistive technology unless the caller explicitly hides it.', rtl: 'Inline content follows surrounding bidi flow.', touch: 'Non-interactive.', responsive: 'Intrinsic and lightweight.',
  ...
  },
  {
    ...
    status: 'candidate', accessibility: 'Static status is ordinary labelled content by default; set announce when a state transition genuinely needs a polite live-region announcement.', rtl: 'Dot and label use logical inline order.', touch: 'Non-interactive.', responsive: 'Intrinsic and wrap-safe.',
  },
  {
    ...
    status: 'candidate', accessibility: 'Built on native progress semantics with an accessible label.', rtl: 'Label/value header follows logical order.', touch: 'Non-interactive.', responsive: 'Track fills available inline size.',
  },
  {
  ...
  },
  {
    ...
    status: 'candidate', accessibility: 'Hidden from the accessibility tree so it does not become fake content.', rtl: 'Direction-neutral geometry.', touch: 'Non-interactive.', responsive: 'Width presets remain container-relative.',
  },
  {
    ...
    status: 'candidate', accessibility: 'Keeps semantic text/action ownership with caller-provided content.', rtl: 'Centered composition is bidi-safe.', touch: 'Optional action consumes the normal component target contract.', responsive: 'Constrained readable copy adapts to narrow containers.',
  },
] as const);
