...
function CursorProbe() {
  ...
  return (
    <output data-testid="cursor">
      {cursor.modality}:{cursor.pointerVisible ? 'visible' : 'hidden'}
    </output>
  );
}
...
function KeyboardDragFixture({ onDrop }: { onDrop: () => void }) {
  const source = useDragSource({
    id: 'source',
    item: { id: 'source', type: 'item', label: 'Source' },
  });
...
describe('UIP04 runtime integration', () => {
  ...
  it('supports keyboard drag selection and drop through the shared runtime', async () => {
    ...
    render(
      <UiRoot>
        <KeyboardDragFixture onDrop={onDrop} />
      </UiRoot>,
    );
