...
export function defineUiIcon(
  ...
  if (!stateNames.length)
    throw new Error('defineUiIcon stateful definitions require stable states.');
  if (!stateNames.includes(definition.defaultState)) {
    throw new Error(
      `defineUiIcon defaultState ${JSON.stringify(definition.defaultState)} is not a declared stable state.`,
    );
  }

  const states: Record<string, UiIconGlyphDefinition> = {};
  for (const state of stateNames) {
    states[state] = normalizeGlyph(
      definition.states[state],
      `defineUiIcon state ${JSON.stringify(state)}`,
    );
  }
  ...
  const transitions = (definition.transitions ?? []).map((transition) => {
    if (!states[transition.from] || !states[transition.to]) {
      throw new Error(
        `defineUiIcon transition ${transition.from} -> ${transition.to} references an unknown stable state.`,
      );
    }
    ...
    if (!transientState)
      throw new Error('defineUiIcon transitions require a non-empty transientState.');
    if (states[transientState]) {
      throw new Error(
        `defineUiIcon transientState ${JSON.stringify(transientState)} must be distinct from stable states.`,
      );
    }
    const key = `${transition.from}\u0000${transition.to}`;
    if (keys.has(key))
      throw new Error(
        `defineUiIcon transition ${transition.from} -> ${transition.to} is duplicated.`,
      );
    keys.add(key);
    return Object.freeze({
      ...
      transient: transition.transient
        ? normalizeGlyph(
            ...
            `defineUiIcon transient state ${JSON.stringify(transientState)}`,
          )
        : undefined,
...
const ICONS = {
  ...
  browser: defineUiIcon({
    paths: [
      'M12 4a8 8 0 1 0 0 16 8 8 0 0 0 0-16Z',
      'M4 12h16',
      'M12 4c3 3.5 3 12.5 0 16',
      'M12 4c-3 3.5-3 12.5 0 16',
    ],
  }),
  ...
  music: defineUiIcon({
    paths: ['M9 18V7l9-2v11M9 18a2.5 2.5 0 1 1-2.5-2.5H9zm9-2a2.5 2.5 0 1 1-2.5-2.5H18'],
  }),
  photos: defineUiIcon({
    paths: [
      'M6 5h12a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a2 2 0 0 1 2-2Z',
      'M7.5 10a1.5 1.5 0 1 0 3 0 1.5 1.5 0 0 0-3 0Z',
      'm6.5 17 4-4 2.6 2.6 1.8-1.8 2.6 3.2',
    ],
  }),
  ...
  search: defineUiIcon({
    paths: ['m17.5 17.5-3.4-3.4', 'M15.5 10.5a5 5 0 1 1-10 0 5 5 0 0 1 10 0Z'],
  }),
  settings: defineUiIcon({
    paths: [
      'M12 9a3 3 0 1 0 0 6 3 3 0 0 0 0-6Z',
      'M12 3.5v2M12 18.5v2M3.5 12h2M18.5 12h2M6 6l1.4 1.4M16.6 16.6 18 18M18 6l-1.4 1.4M7.4 16.6 6 18',
    ],
  }),
  software: defineUiIcon({
    paths: ['m12 3 8 4.5v9L12 21l-8-4.5v-9zm0 0v9m8-4.5-8 4.5-8-4.5M12 12v9'],
  }),
...
type IconSvgProps = Omit<
  PrimitiveSvgProps,
  'role' | 'aria-label' | 'aria-hidden' | 'focusable' | 'onAnimationStart' | 'onAnimationEnd'
>;
...
type IconSharedProps = IconSvgProps &
  IconReservedSvgProps & {
...
function readIconTransitionBudgetMs(element: Element) {
  ...
  return (longest > 0 ? longest : ICON_TRANSITION_FALLBACK_MS) + ICON_TRANSITION_SETTLE_SLACK_MS;
}
  ...
  return (
    ...
    >
      {activeTransition ? (
        ...
        <g
          className="ui-icon__layer ui-icon__layer--stable"
          data-oxs-icon-stable={activeVisual.stableState}
        >
