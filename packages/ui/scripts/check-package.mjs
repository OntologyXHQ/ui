...
for (const [subpath, target] of Object.entries(pkg.exports ?? {})) {
  ...
  for (const value of values)
    if (typeof value === 'string' && value.includes('/src/'))
      failures.push(`${subpath} leaks source export ${value}`);
}
for (const rel of [
  'dist/index.js',
  'dist/advanced.js',
...
for (const rel of ['dist/index.js', 'dist/advanced.js', 'dist/icons.js']) {
  try {
    ...
    if (/['"][^'"\n]+\.css['"]/.test(javascript)) {
      failures.push(
        `${rel} must be stylesheet-neutral; consumers import @ontologyx/ui/styles.css explicitly`,
      );
    }
  } catch {}
}
...
if (
  !iconsExport ||
  typeof iconsExport === 'string' ||
  iconsExport.import !== './dist/icons.js' ||
  iconsExport.types !== './dist/icons.d.ts'
) {
  failures.push(
    'package must expose ./icons as the optional built ESM/declaration icon-pack subpath',
  );
}
...
try {
  ...
  if (types.includes('./styles/index.css'))
    failures.push('dist/index.d.ts leaks the source stylesheet path');
} catch {}
const distFiles = [];
...
for (const file of distFiles) {
  if (/(__tests__|\.test\.|\.docs\.)/.test(file))
    failures.push(`test/docs artifact leaked into dist: ${path.relative(root, file)}`);
}
try {
  ...
  if (mainEntry.includes(canvasMarker)) {
    failures.push(
      'optional OX Canvas renderer was inlined into dist/index.js instead of remaining lazy',
    );
  }
  const javascriptChunks = distFiles.filter(
    (file) => file.endsWith('.js') && path.relative(root, file) !== 'dist/index.js',
  );
  let canvasChunkFound = false;
  for (const file of javascriptChunks) {
    const source = await readFile(file, 'utf8');
    if (source.includes(canvasMarker)) {
      canvasChunkFound = true;
      break;
    }
  }
...
const total = (await Promise.all(distFiles.map((file) => stat(file)))).reduce(
  (sum, item) => sum + item.size,
  0,
);
console.log(
  `@ontologyx/ui package artifact check passed: ${distFiles.length} files · ${total} bytes.`,
);
