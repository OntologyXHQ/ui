...
for (const entry of acceptedLayout) {
  for (const prop of entry.props ?? []) {
    if (forbiddenProps.has(prop.name) && !(entry.exportName === 'Container' && prop.name === 'width')) {
      issues.push(`${entry.exportName}.${prop.name}: layout API bypasses logical/typed composition policy`);
    }
  }
  if (!entry.examples?.length) issues.push(`${entry.exportName}: accepted layout primitive has no dedicated Studio example`);
...
for (const physical of ['marginLeft', 'marginRight', 'paddingLeft', 'paddingRight', "'left'", "'right'"]) {
  if (layoutSource.includes(physical)) issues.push(`Layout.tsx contains forbidden physical-direction API vocabulary: ${physical}`);
}

if (!layoutSource.includes("export type GridColumns = 'auto-fit' | GridColumnCount")) {
  issues.push('Grid must expose finite fixed columns plus intrinsic auto-fit rather than arbitrary track strings');
}
...
if (!layoutCss.includes('repeat(auto-fit, minmax(min(100%, var(--ui-grid-min-column)), 1fr))')) {
  issues.push('Grid auto-fit CSS must use bounded minmax tracks that cannot force page-level overflow');
}
...
if (layoutSource.includes("'compact'")) {
  issues.push('Container must use semantic readable/content/wide/full vocabulary; compact is device-like/ambiguous');
}

for (const logicalInset of ['inline', 'block', 'inlineStart', 'inlineEnd', 'blockStart', 'blockEnd']) {
  if (!new RegExp(`\\b${logicalInset}\\?:\\s*SpaceToken`).test(layoutSource)) {
    issues.push(`Inset logical override missing: ${logicalInset}`);
  }
}
if (!layoutCss.includes('--ui-inset-inline-start: var(--ui-inset-inline);') || !layoutCss.includes('padding-inline-start: var(--ui-inset-inline-start);')) {
...
for (const edge of ['block-start', 'inline-end', 'block-end', 'inline-start']) {
  if (!layoutSource.includes(`'${edge}'`)) issues.push(`SafeArea logical edge type missing: ${edge}`);
  if (!layoutCss.includes(`.ui-safe-area-edge-${edge} {`)) issues.push(`SafeArea CSS edge ownership missing: ${edge}`);
}
if (/SafeAreaEdge[^\n]*\b(left|right|top|bottom)\b/.test(layoutSource)) {
  issues.push('SafeArea public edge contract must remain logical, never physical');
}
if (/--oxs-occlusion-/.test(layoutCss.slice(layoutCss.indexOf('.ui-safe-area'), layoutCss.indexOf('.ui-spacer')))) {
  issues.push('SafeArea must consume persistent safe-area variables only, never transient occlusion');
}

if (!layoutSource.includes("export type SpacerAxis = 'inline' | 'block'")) {
  issues.push('Spacer must reserve exactly one logical axis');
}
if (/export type SpacerProps\s*=\s*PrimitiveHtmlProps/.test(layoutSource) || /\.\.\.props/.test(layoutSource.slice(layoutSource.indexOf('export function Spacer')))) {
  issues.push('Spacer must not expose a DOM prop bag that can make its aria-hidden node focusable/semantic');
}
if (!layoutSource.includes('aria-hidden="true"')) {
  issues.push('Spacer must hard-code aria-hidden=true');
}

for (const legacySelector of ['.ui-grid--tile', '.ui-grid--card', '.ui-grid--wide', '.ui-container--compact', '.ui-safe-area--inline', '.ui-spacer--axis-both']) {
  if (layoutCss.includes(legacySelector)) issues.push(`legacy pre-certification layout selector remains: ${legacySelector}`);
}
