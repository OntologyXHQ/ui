...
- privileged System surfaces stay host-neutral: notification/quick-settings/OSD/command/lock/keyboard code remains Component/System-only, owns no browser/native/backend authority, keyboard repeat is owner-Window scoped and visibility/session-cancelled, edge-reaching surfaces keep explicit logical safe-area/occlusion ownership, and real OXS verification is required before UIR15 can close.
...
- UIR15 privileged System certification proves stable notification/command/keyboard identity boundaries, noninteractive caller-timed OSD semantics, owner-realm keyboard repeat cancellation, privileged-host mounting, logical safe-area versus transient-occlusion handling (including keyboard self-occlusion isolation), RTL/coarse-pointer/reduced-motion behavior and accessibility without moving host/native authority into React;
