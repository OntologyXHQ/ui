...
- detail pages use public `AppBar`, typography, surfaces and layout primitives in a single stacked reading flow while rendering generated import/API/JSDoc/deprecation, examples, playground and cross-axis guidance; Studio no longer requires tab chrome to navigate its own documentation.
...
- the global environment toolbar owns theme/custom-theme, direction, density, motion, modality, pointer precision, viewport, content-container and safe-area/occlusion simulation at the outer preview `UiRoot`, so examples inherit the same environment without bespoke wiring; the toolbar itself runs inside a nested stable LTR/comfortable control `UiRoot` so preview direction/density cannot make its own reversal controls unreachable.
